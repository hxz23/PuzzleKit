...
class PuzzleListDivider: UICollectionViewCell, PuzzleCellProtocol {
    let v = UIView()

    func update(_ object: AnyObject) {
        guard let vm = object as? PuzzleListDividerModel else { return }
        if v.superview == nil {
...
open class PuzzleListDividerModel: PuzzleListCellModel {
