...
class WrapColorCell: UICollectionViewCell, PuzzleCellProtocol {
    ...
    var viewModel: WrapColorCellModel?
    func update(_ object: AnyObject) { // 需要实现 PuzzleCellProtocol
        guard let vm = object as? WrapColorCellModel else { return }
        viewModel = vm
...
class WrapColorCellModel: PuzzleWrapCellModel {
    let index: Int
    let color: UIColor
